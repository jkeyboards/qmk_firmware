	...
	switch (keycode) {
		case DISCORD:
			if (record->event.pressed) {
			    SEND_STRING(SS_TAP(X_LGUI) SS_DELAY(500) "discord" SS_DELAY(500) SS_TAP(X_ENT));
			} else {
 ...
 */
 [_BL] = LAYOUT_ortho_5x4(
   DISCORD, KC_F13,  KC_F14,   LT(_FN, KC_BSPC),
   ORIGIN,  APEX,    KC_9,     KC_PMNS,
   VIA,     KC_5,    KC_6,     KC_PPLS,
